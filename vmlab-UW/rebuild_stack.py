...
def CreateStack3DBlock(x, name = 'stackblock'):
    x = klayers.Conv3D(256, (3,3,3), padding='valid', name=name+'_conv1')(x)
    x = klayers.Conv3D(256, (3,3,3), padding='valid', name=name+'_conv2')(x)
    dilated_layers=[]
    dilated_layers.append(x)
    x = klayers.Conv3D(256, (3,3,3), padding='same', name=name+'_conv3',dilation_rate = (2,2,2))(x)
    dilated_layers.append(x)
    x = klayers.Conv3D(256, (3,3,3), padding='same', name=name+'_conv4',dilation_rate = (4,4,4))(x)
    dilated_layers.append(x)
    x = klayers.Conv3D(256, (3,3,3), padding='same', name=name+'_conv5',dilation_rate = (8,8,8))(x)
    dilated_layers.append(x)

    return klayers.add(dilated_layers, name = name+'_add')
...
def Construct3DUnetModel(input_images, nclasses, use_bn = True, use_dropout = True):
    ...
    with name_scope("stackblock"):
        x = CreateStack3DBlock(x, name = 'stackblock')
